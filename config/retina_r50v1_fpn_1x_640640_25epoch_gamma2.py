...
def get_config(is_train):
    ...
    class OptimizeParam:
        ...
        class schedule:
            begin_epoch = 0
            end_epoch = 25
            lr_iter = [14657 * 15 * 16 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image),
                       14657 * 20 * 16 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image)]

        class warmup:
            type = "gradual"
            lr = 0.001 / 8 * len(KvstoreParam.gpus) * KvstoreParam.batch_image
            iter = 14657 * 1 * 16 // (len(KvstoreParam.gpus) * KvstoreParam.batch_image)
